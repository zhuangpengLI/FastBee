...
public abstract class AbstractQuartzJob implements Job
{
    ...
    protected void after(JobExecutionContext context, DeviceJob deviceJob, Exception e)
    {
        ...
        int jobType = deviceJob.getJobType().intValue();
        //调整一下 不存储设备名称了  存储设备id  还应该存储类型
        if(jobType==1) {
        	sysJobLog.setInvokeTarget(deviceJob.getDeviceId()+"");
        }else if(jobType==3) {
        	sysJobLog.setInvokeTarget(deviceJob.getSceneId()+"");
        }else if(jobType==2) {
        	sysJobLog.setInvokeTarget(deviceJob.getAlertId()+"");
        }else {
        	sysJobLog.setInvokeTarget("无效目标");
        }
        sysJobLog.setJobType(jobType);
