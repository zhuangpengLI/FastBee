<template>
  <div style="padding: 6px">
    <el-card style="margin-bottom: 6px">
      ...
      >
        <el-form-item label="网关名称" prop="deviceName">
          <el-input
        ...
        <el-form-item label="网关编号" prop="serialNumber">
          <el-input
    ...
    <el-table v-loading="loading" :data="deviceList">
      ...
      <!-- <el-table-column label="网关ID" align="center" prop="deviceId" /> -->
      <el-table-column label="序号" align="center" prop="index" width="80" />
      <el-table-column label="网关名称" align="center" prop="deviceName" />
      ...
      <el-table-column label="网关编号" align="center" prop="serialNumber" />
      ...
      <!-- <el-table-column label="wifi信号强度" align="center" prop="rssi" />
      <el-table-column label="是否启用设备影子(0=禁用，1=启用)" align="center" prop="isShadow" /> -->
      <el-table-column
        label="设备所在地址"
        align="center"
        prop="networkAddress"
      />
      <!-- <el-table-column label="是否自定义位置" align="center" prop="isCustomerLocation" /> -->
      <!-- <el-table-column label="设备入网IP" align="center" prop="networkIp" />
      <el-table-column label="物模型值" align="center" prop="thingsModelValue" />
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            icon="el-icon-link"
            :style="{
              display: scope.row.belongFamilyId === null ? 'none' : '',
            }"
            @click="handleUnbinding(scope.row)"
            v-hasPermi="['iot:gateway:unbinding']"
            >解绑</el-button
          >
          <!-- <el-button
            size="mini"
          ...
          <el-button
            ...
            v-hasPermi="['iot:gateway:restart']"
            >远程重启</el-button
          >
          <el-button
            size="mini"
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-search"
            @click="handleOTA(2, scope.row)"
            v-hasPermi="['iot:gateway:OTA']"
            >OTA升级</el-button
          >
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-table-column label="类型" align="center" prop="alertLevel" />
        <el-table-column label="备注" align="center" prop="remark" />
        <el-table-column
          label="上报时间"
          align="center"
    ...
    >
      ...
      >
        ...
        <el-table-column label="固件路径" align="center" prop="firmwarePath" />
        <el-table-column label="升级版本" align="center" prop="otaVersion" />
        <el-table-column label="升级状态" align="center" prop="status">
          <template slot-scope="scope">
            <dict-tag
              :options="dict.type.iot_OTA_status"
              :value="scope.row.status"
            />
          </template>
        </el-table-column>
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="固件上传" prop="firmwarePath">
          <fileUpload
            ref="file-upload"
    ...
    <el-dialog title="实时监测" :visible.sync="open" width="800px">
      ...
      <el-form :inline="true" label-width="100px">
        ...
        <el-form-item>
          <el-button
            ...
            size="mini"
            @click="updateMonitorParameters()"
            style="margin-left: 30px"
            :disabled="monitorDevice.status != 3"
            >更 新</el-button
          >
        </el-form-item>
      </el-form>
      <el-row
        ...
        element-loading-text="正在接收设备数据，请耐心等待......"
        element-loading-spinner="el-icon-loading"
        element-loading-background="rgba(0, 0, 0, 0.8)"
      >
        <el-col
          :span="12"
          v-for="(item, index) in monitorThings"
          :key="index"
          style="margin-bottom: 20px"
        >
          <el-card
            shadow="hover"
            :body-style="{ paddingTop: '10px', marginBottom: '-20px' }"
          >
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      console.log("xxx1", this.form.firmwarePath);
      if (this.form.firmwarePath ===  undefined) {
        this.$message.error('文件路径不能为空');
        return
      }
      else if(this.form.remark ===  undefined){
        this.$message.error('升级说明不能为空');
        return
      }else{
        this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.deviceId != null) {
            upgradeGateway(this.form).then((response) => {
              this.$modal.msgSuccess(response.msg);
              this.OTAopen = false;
              this.getList();
            });
          } else {
            batchUpgradeGateway(this.form).then((response) => {
              this.$modal.msgSuccess(response.msg);
              this.OTAopen = false;
              this.getList();
            });
    ...
    handleOTA(type, row) {
      ...
      if (type === 2) {
        this.form.deviceId = row.deviceId || this.deviceId;
    ...
    getFilePath(data) {
      ...
      // const list = Array.isArray(data.name) ? data.name : data.name.split('.');
      const dd = data.name.substring(0, data.name.lastIndexOf("."));
      this.form.version = dd;
      if(data.response.url.startsWith("http")){
        this.form.firmwarePath = data.response.url;
      }else{
        if (process.env.NODE_ENV === "production") {
          ...
          this.form.firmwarePath =
            process.env.VUE_APP_BASE_API + data.response.url;
        }
      }
      console.log("xxx1", this.form.firmwarePath);
      if(this.form.firmwarePath.startsWith("https")){
        this.form.firmwarePath = this.form.firmwarePath.replace("https://","http://")
      }
      console.log("xxx2", this.form.firmwarePath);
    },
    handleUploadSuccess(res, file) {
      this.$message.success("上传成功");
      this.fileList.push({ name: res.fileName, url: res.fileName });
      // this.form.otaVersion=file.name
    },
    // 文件提交处理
    submitUpload() {
    ...
    handleFileSuccess(response, file, fileList) {
      ...
      this.form.firmwarePath = process.env.VUE_APP_BASE_API + response.url;
      this.$modal.msgSuccess(response.msg);
    },
    // 文件下载处理
    handleDownload(row) {
      window.open(process.env.VUE_APP_BASE_API + row.firmwarePath);
    },
    /** 发布物模型 类型(1=属性，2=功能) */
    ...
    mqttPublish(type, device, model) {
      ...
      if (type == 1) {
        ...
        // 实时监测
        topic =
          "/" + device.productId + "/" + device.serialNumber + "/monitor/get";
        message =
          '{"count":' +
          model.value +
          ',"interval":' +
          ...
          for (let i = 0; i < this.deviceList.length; i++) {
            if (this.deviceList[i].serialNumber == deviceNum) {
              for (let j = 0; j < message.length; j++) {
                ...
                for (
                  let k = 0;
                  k < this.deviceList[i].boolList.length && !isComplete;
                  k++
                ) {
                ...
                for (
                  let k = 0;
                  k < this.deviceList[i].enumList.length && !isComplete;
                  k++
                ) {
                ...
                for (
                  let k = 0;
                  k < this.deviceList[i].stringList.length && !isComplete;
                  k++
                ) {
                ...
                for (
                  let k = 0;
                  k < this.deviceList[i].arrayList.length && !isComplete;
                  k++
                ) {
                ...
                for (
                  let k = 0;
                  k < this.deviceList[i].integerList.length && !isComplete;
                  k++
                ) {
                ...
                for (
                  let k = 0;
                  k < this.deviceList[i].decimalList.length && !isComplete;
                  k++
                ) {
                ...
                for (
                  let k = 0;
                  k < this.deviceList[i].readOnlyList.length && !isComplete;
                  k++
                ) {
                  if (this.deviceList[i].readOnlyList[k].id == message[j].id) {
                    this.deviceList[i].readOnlyList[k].shadow =
                      message[j].value;
                    isComplete = true;
                    break;
                  }
    ...
    getList() {
      ...
      if (null != this.daterangeActiveTime && "" != this.daterangeActiveTime) {
        this.queryParams.params["beginActiveTime"] =
          this.daterangeActiveTime[0];
        this.queryParams.params["endActiveTime"] = this.daterangeActiveTime[1];
      }
      // this.getGroupList();
      listGatewayShort(this.queryParams).then((response) => {
        ...
        for (let i = 0; i < this.deviceList.length; i++) {
          ...
          if (readonlyCount == 0) {
            ...
            //保留6个监测值
            this.deviceList[i].readOnlyList = this.deviceList[
              i
            ].readOnlyList.slice(0, 6);
          }
        }
        // 订阅消息
    ...
    handleUnbinding(row) {
      ...
      this.$modal
        .confirm("确认是否解绑sn:" + serialNumber + "的网关？")
        .then(function () {
          return unbindingGateway(deviceIds);
        })
        .then(() => {
          this.getList();
    ...
    handlelog(row) {
      // this.reset();
      this.query2.deviceId = row.deviceId || this.deviceId;
      this.deviceId = row.deviceId || this.deviceId;
      logList(this.query2).then((response) => {
        this.logList = response.rows;
        this.logList.map((item, index) => {
          item.index =
            (this.query2.pageNum - 1) * this.query2.pageSize + index + 1;
        });
        // console.log(this.sceneDeviceList);
    ...
    getCacheThingsModdel(productId) {
      ...
      cacheJsonThingsModel(productId).then((response) => {
        let thingsModel = JSON.parse(response.data);
        // 筛选监测数据
        this.monitorThings = thingsModel.properties.filter(
          (item) => item.isMonitor == 1
        );
        // 监测数据集合初始化
        this.dataList = [];
        ...
        option = {
          title: {
            ...
            text:
              ...
              " 单位 " +
              (this.monitorThings[i].datatype.unit != undefined
                ? this.monitorThings[i].datatype.unit
                : "无") +
              "）",
            textStyle: {
              fontSize: 14,
