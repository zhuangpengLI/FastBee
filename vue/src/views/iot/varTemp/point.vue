  ...
  <div style="padding:6px;">
    ...
    <el-card style="padding:-10px;padding-bottom:30px;">
      <el-row :gutter="0">
        <el-col :span="8">
          <el-form :inline="true">
            <el-form-item>
              <el-button style="margin-top: 13px" size="mini" type="primary" @click="refresh">刷新</el-button>
            </el-form-item>
          </el-form>
          <div style="border: 1px solid #e6ebf5;">
            ...
            <el-table v-loading="loading" :data="salveList" size="mini" highlight-current-row
              ...
              <el-table-column prop="timer" header-align="center" align="left" label="采集频率">
                <template slot-scope="scope">
                  <dict-tag :options="dict.type.iot_modbus_poll_time" :value="scope.row.timer"/>
                </template>
              </el-table-column>
        ...
        <el-col :span="16">
          <div class="container-02">
            ...
            <el-table v-loading="sLoadoing" :data="pointList" size="mini">
              ...
              <el-table-column prop="templateName" label="物模型名称" width="150">
              </el-table-column>
              <el-table-column prop="regAddr" label="寄存器地址" width="95"/>
              ...
              <el-table-column prop="datatype" label="数值类型" width="100px"/>
              <el-table-column prop="isReadonly" label="读写类型" width="75">
                <template slot-scope="scope">
              ...
              <el-table-column prop="formula" label="计算公式" width="95"/>
    ...
    <el-dialog :title="title" :visible.sync="openViewVarTemp" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <!--            </el-form-item>-->
        <el-form-item label="功能码" prop="code" size="small">
          <el-select  v-model="form.code" placeholder="请选择"
                     style="width: 200px; display: inline-block; padding-right: 10px">
            <el-option v-for="dict in dict.type.iot_modbus_status_code" :key="dict.value" :label="dict.label"
                       :value="dict.value">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="寄存器/线圈" prop="regStr" >
          <el-input v-model="form.regStr" placeholder="请输入地址(10进制)"
                    style="width: 200px; display: inline-block; padding-right: 10px"/>
        </el-form-item>
        <el-form-item label="读取个数" prop="quantity" size="small">
          <template slot="label">
            <span>读取个数</span>
            ...
            <el-input v-model="form.quantity" placeholder="单个数据点读取寄存器或线圈的个数" style="width: 200px; display: inline-block; padding-right: 10px"/>
        </el-form-item>
        <el-form-item label="解析类型" prop="parseType" size="small">
          <el-select  v-model="form.parseType" placeholder="Modbus解析类型"
                      style="width: 200px; display: inline-block; padding-right: 10px">
            <el-option v-for="dict in dict.type.iot_modbus_data_type" :key="dict.value" :label="dict.label"
                       :value="dict.value">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="模型特性" prop="property">
          <template slot="label">
            ...
            <el-tooltip style="cursor: pointer;" effect="light" placement="top">
              <div slot="content">
                图标展示: 在运行状态以图标形式展示<br/>
                只读数据: 只读数据点位/读写数据点位<br/>
                历史存储：勾选后在历史数据记录,历史数据处展示<br/>
              </div>
              <i class="el-icon-question"/>
            </el-tooltip>
          </template>
    ...
    <el-dialog :title="title" style="" :visible.sync="openViewSalve" width="550px" append-to-body>
      <el-form ref="addSavleFrom" :model="addSavleFrom" :rules="salveRules" label-width="130px" size="mini"
        ...
        <el-form-item label="寄存器起始地址" prop="addrStart" size="small">
          <el-input v-model="addSavleFrom.addrStart" placeholder="寄存器起始地址(10进制),例如 0x000A 填写 10 "/>
        </el-form-item>
        <el-form-item label="寄存器结束地址" prop="addrEnd" size="small">
          <el-input v-model="addSavleFrom.addrEnd" placeholder="寄存器结束地址(10进制),例如 0x00AA 填写 170"/>
        </el-form-item>
        <el-form-item label="批量采集频率" prop="timer" size="small">
          <el-select v-model="addSavleFrom.timer" placeholder="请选择" style="width:100%;">
            <el-option v-for="dict in dict.type.iot_modbus_poll_time" :key="dict.value" :label="dict.label"
                       :value="dict.value">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="组包寄存器最大数量" prop="packetLength" v-if="selectOpen" label-width="170px" type="number">
          <template slot="label">
            <span>组包寄存器最大数量</span>
            <el-tooltip style="cursor: pointer;" effect="light" placement="top">
              <div slot="content">云端采集的协议支持配置组包寄存器最大的数量; <br/>
                <div style="height: 10px"></div>
                <b>不填写: </b>每次随机轮询32~64个寄存器（2 Byte）<br/>Modbus协议回复包没有起始地址，可以解决网络延迟<br/>或回复超时解析变量错位问题；<br/>
                <div style="height: 10px"></div>
                <b>填写: </b>每次按填写的数值轮询寄存器，通过云端的智<br/>能化算法，可以降低网络延迟或回复超时解析变量错<br/>位问题的概率;
              </div>
              <i class="el-icon-question"/>
            </el-tooltip>
          </template>
          <el-input prop="packetLength" v-model="addSavleFrom.packetLength" placeholder="批量读取寄存器的个数(32-64)"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="cancel">取 消</el-button>
    ...
    <!-- 采集点对应所有产品 -->
    <el-dialog :title="title" :visible.sync="openViewProduct" width="550px" append-to-body>
...
export default {
  name: "Point",
  dicts: ['data_collect_type', 'iot_modbus_status_code', 'iot_modbus_data_type', 'iot_modbus_poll_time', 'iot_data_read_write','iot_yes_no',"iot_things_type"],
  components: {
    thingsParameter
  },
  data() {
  ...
  created() {
    ...
    this.addSavleFrom.pollingMethod = this.$route.query && this.$route.query.pollingMethod;
  ...
  methods: {
    ...
    /** 提交按钮 */
    submitFormSlave() {
      this.addSavleFrom.pollingMethod = this.content.pollingMethod;
    ...
    /*导入模板*/
    importTemplate() {
      this.download('iot/template/temp', {}, `采集点_${new Date().getTime()}.xlsx`)
    },
    // 提交上传文件
    submitFileForm() {
