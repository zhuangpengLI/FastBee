    ...
    <el-card style="padding-bottom: 100px">
      ...
      >
        ...
        <el-table-column
          label="场景指令数"
          align="center"
          prop="sceneDeviceCount"
        />
         <el-table-column label="场景开关" align="center" prop="enabled">
              <template slot-scope="scope">
                  <dict-tag :options="dict.type.iot_is_enable" :value="scope.row.enabled" />
              </template>
          </el-table-column>
        <el-table-column label="所属家庭" align="center" prop="familyName" />
        <el-table-column
          label="创建时间"
          align="center"
          prop="createTime"
        ...
        >
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              icon="el-icon-search"
              @click="handlesceneDeviceCount(scope.row)"
              v-hasPermi="['iot:scene:search']"
              >查看</el-button
            >
            <el-button
              size="mini"
              type="text"
              icon="el-icon-search"
              @click="handleactions(scope.row)"
              v-hasPermi="['iot:scene:actions']"
              >执行记录</el-button
            >
            <el-button
              size="mini"
              ...
              v-hasPermi="['iot:scene:configure']"
              >定时配置</el-button
            >
            <!-- <el-button
              size="mini"
      ...
      <el-dialog
        :title="title"
        :visible.sync="sceneDeviceCountopen"
        width="900px"
        append-to-body
      >
        <el-table
        ...
        >
          ...
          <el-table-column label="执行命令" align="center" prop="actionsName" />
        </el-table>

        <pagination
          v-show="sceneDeviceCounttotal > 0"
          :total="sceneDeviceCounttotal"
          :page.sync="query.pageNum"
          :limit.sync="query.pageSize"
          @pagination="handlesceneDeviceCount"
        />
      </el-dialog>
      <!--查看执行记录对话框 -->
      <el-dialog
        :title="title"
        :visible.sync="actionsopen"
        width="900px"
        append-to-body
      >
        <el-table
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span v-if="scope.row.runType===1">手动执行</span>
              <span v-if="scope.row.runType===2">定时执行{{scope.row.jobName}}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="执行人"
          ...
          >
            <template slot-scope="scope">
              <span>{{ scope.row.nickName }}--{{ scope.row.phonenumber }}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="执行设备数/成功数"
          ...
          >
            <template slot-scope="scope">
              <span>{{ scope.row.deviceCount }}/{{ scope.row.sucDeviceCount }}</span>
            </template>
          </el-table-column>
        </el-table>

        <pagination
          v-show="actionstotal > 0"
          :total="actionstotal"
              ...
              <el-form-item label="执行动作">
                ...
                >
                  ...
                  <el-row v-if="item.source == 3">
                    <el-col :span="4">
                      <el-select
                        v-model="item.alertLevel"
                        placeholder="告警级别"
                        size="small"
                      >
                        <el-option
                          v-for="dict in dict.type.iot_alert_level"
                          :key="dict.value"
                          :label="dict.label"
...
export default {
  ...
  name: "Scene",
  dicts: ["iot_alert_level","iot_is_enable"],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    /** 查看操作 */
    handlesceneDeviceCount(row) {
      this.reset();
      this.query.sceneId = row.sceneId || this.sceneId;
      this.sceneId = row.sceneId;
      deviceList(this.query).then((response) => {
        this.sceneDeviceList = response.rows;
        this.sceneDeviceList.map((item, index) => {
          item.index =
            (this.query.pageNum - 1) * this.query.pageSize + index + 1;
        });
        console.log(this.sceneDeviceList);
        this.sceneDeviceCounttotal = response.total;
        this.sceneDeviceCountopen = true;
        this.title = "查看指令设备";
      });
    },
    /** 执行记录操作 */
    handleactions(row) {
      this.reset();
      this.query1.sceneId = row.sceneId || this.sceneId;
      this.sceneId = row.sceneId;
      getSceneRunRecord(this.query1).then((response) => {
        this.actionsList = response.rows;
        this.actionsList.map((item, index) => {
          item.index =
            (this.query1.pageNum - 1) * this.query1.pageSize + index + 1;
        });
        console.log(this.actionsList);
        this.actionstotal = response.total;
    ...
    handleconfigure(row) {
      this.reset();
      this.query2.sceneId = row.sceneId || this.sceneId;
      this.sceneId = row.sceneId;
      getSceneJobList(this.query2).then((response) => {
        this.configureList = response.rows;
        this.configureList.map((item, index) => {
          item.index =
            (this.query2.pageNum - 1) * this.query2.pageSize + index + 1;
        });
        console.log(this.configureList);
        this.configuretotal = response.total;
    ...
    gentCronExpression() {
      ...
      this.form.triggers[this.triggerIndex].cronExpression =
        "0 " + minute + " " + hour + " ? * " + week;
    },
  },
};
